...
- dkfgrain/fgrainstr~ - granular stretcher using tabread~
- dkfgrain/fgrainstr2~ - granular stretcher using tabplay~ to avoid onset truncation error (no transposition)
- dkfgrain/fgraintrig~ - triggered granular stretcher (requires fgrainstr~)
- dkfgrain/fgrainxfade~ - granular stretcher/crossfader
- dkfilter/allpass_van~ - vanilla implementation of an all-pass filter using y[n] = gx[n]+x[n-d]-gy[n-d]
- dkfilter/comb_van~ - vanilla implementation of Max's comb~ comb filter using y[n] = ax[n]+bx[n-d]+cy[n-d]
- dkfilter/teeth_van~ - vanilla implementation of Max's teeth~ comb filter using y[n] = ax[n]+bx[n-FFD]+cy[n-FBD] featuring control of all feedforward and feedback parameters
